...
%ftt= strcat('exp(-x/a)*cos(x/b+c)/cos(c)'); %Oct 1 edit 
ftt= strcat('exp(-a*x)*cos(b*x+c)/cos(c)'); %Oct 1 edit 
ft = fittype( sprintf('%s',ftt));
opts = fitoptions( ft );
opts.Display = 'Off';
%opts.StartPoint = [0.28, 0.55, 1.24]; % beginning parameters - amp, mu, std.
opts.StartPoint = [3.57 1.82 1.24]; %Oct 1 edit 
opts.Lower = [0.001,-inf,-inf];
[fit.f,fit.gof]=fit(dx_nond2_5loc_all(include),cxt_dxwidth_5loc_all(include),ft, opts);

fit.dx_fit = 0:0.01:3;
fit.cxt_fit = exp(-fit.f.a*fit.dx_fit).*cos(fit.f.b*fit.dx_fit+fit.f.c)/cos(fit.f.c);
